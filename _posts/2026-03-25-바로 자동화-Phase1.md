...
이를 위해 우리는 단순히 텍스트를 생성하는 수준을 넘어,  
**코드베이스를 읽고 문맥을 반영한 명세 초안을 만들고, 사람이 검토·승인한 뒤 최종 결과를 확정하는 구조**를 지향한다.

특히 이 시스템은 **Slack을 Approval Gate로 사용하는 human-in-the-loop 방식**을 전제로 한다.   따라서 기능 명세 자동화 시스템은 처음부터 **Slack 친화적인 상호작용 구조**를 갖추어야 한다.

즉, 이 시스템은 다음 조건을 만족해야 한다.
...
**본 시스템에서는 에이전트를 전체 시스템의 주체가 아니라, workflow 내부의 실행 단위로 제한하는 것이 적절하다.**

앞서 보았듯이 ‘에이전트’라는 개념은 매우 넓고 모호하다.  
어떤 경우에는 전체 작업을 자율적으로 수행하는 orchestrator를 의미하고,  
어떤 경우에는 특정 단계에서만 호출되는 LLM 기반 작업 단위를 의미한다.

본 문서에서는 이 중 **LLM agent** 개념을 사용하되,  
그 역할을 **시스템 전체를 통제하는 주체가 아니라 각 step 내부에서 제한적으로 동작하는 실행 단위**로 정의한다.

핵심 질문은 이것이다.

**에이전트에게 어디까지 자율성을 부여할 것인가?**

이에 대해 본 시스템은 다음과 같이 판단한다.
...
가장 먼저 떠올릴 수 있는 방식은, 에이전트에게 전체 작업을 위임하는 것이다.  
...
더 중요한 점은, 이 모든 과정이 정말로 에이전트의 고차원 추론이 필요한 영역이냐는 것이다.  
본 시스템에서는 그렇지 않다고 본다.
...
이러한 흐름은 충분히 **코드로 명시할 수 있는 절차적 로직**이며,  
굳이 에이전트의 자율적 판단에 맡길 이유가 크지 않다.
...
실제 동작 흐름은 ‘EC2 애플리케이션이 workflow를 제어하고, 에이전트는 내부 작업을 수행하며, Slack은 검토·승인 인터페이스로 작동하는 형태’로 구체화할 수 있다.
...
이 구조의 핵심은 **승인 이전에는 시스템이 다음 단계로 넘어가지 않는다는 점**이다.

사용자가 Slack에서 요청을 입력하면, 애플리케이션은 workflow를 시작하고 내부적으로 에이전트 작업을 수행한다.  
이때 에이전트는 GitHub MCP 등을 활용해 관련 코드와 문서를 읽고 분석할 수 있다.

이후 시스템은 생성된 기능 명세 초안을 Slack으로 전달하고, 사용자에게 검토를 요청한다.  
사용자는 이 초안을 보고 수정사항을 남기거나, 최종 승인할 수 있다.

- 수정 요청이 들어오면 명세를 다시 생성한다.
- 재검토가 끝나면 다시 Slack에서 확인을 받는다.
- 최종 승인이 내려진 뒤에만 GitHub issue 생성이 수행된다.

즉, **Slack을 명시적인 Approval Gate**로 사용하여 **Human-in-the-loop**를 보장한다.
...
**문제 1. 토큰 비용이 지나치게 크다**
**repository tree를 직접 탐색하는 방식은 너무 많은 토큰을 소모한다.**
...
물론 이를 프롬프트로 더 강하게 제한할 수는 있다.  
하지만 조건이 늘어날수록 프롬프트는 길어지고, 에이전트에게 **일관된 방식으로 동작할 것을 강제하기가 더 어려워진다.**

또 다른 문제는 시작점을 특정 모듈로 고정하는 방식도 근본적인 해결책이 아니라는 점이다.
...
무엇보다 이 방식은 **느리고 비싸다.**  
실제 탐색 과정에서 평균적으로 약 10만 토큰을 사용했다.
...
하지만 지금 이 단계에서 필요한 것은 파일 단위 수정 계획이 아니라,  
**무엇을 어떤 도메인 관점에서 정의해야 하는가**에 대한 명세 수준의 판단이다.

즉, 현재 시점에서 필요한 것은 다음이다.
...
`ENCYCLOPEDIA.md`는 서비스 내에서 사용되는 bounded context를 정의한 문서이며,  
에이전트들이 작업을 판단할 때 참조하는 **단일 기준점(single source of truth)** 이다.
...
예를 들어 `"회원가입 기능 추가"` 와 같은 모호한 입력이 들어오더라도,  
이를 서비스가 사용하는 bounded context 단위로 해석할 수 있다.
